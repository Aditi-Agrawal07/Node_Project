...
const saveProducts = async (req, res) => {
  try {
    ...
    const products = await Product.create({
      ...reqBody,
      product_image: uploadedFile.url,
    });
...
const getCount = async (req, res) => {
  try {
    ...
    const categoryCount = await productService.getCount();
    if (!categoryCount)
      return res.status(404).json({ hasError: true, massage: "Not Found" });
    return res.status(200).json({ hasError: false, categoryCount });
  } catch (err) {
    console.log(err);
  }
};
